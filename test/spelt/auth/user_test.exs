defmodule Spelt.Auth.UserTest do
  ...
  describe "(User)-[AUTHENTICATED_AS]->(Session)" do
    ...
    test "with multiple related Sessions, returns an array of all" do
      ...
      {:ok, _} =
        Spelt.Repo.Relationship.create(%AuthenticatedAs{start_node: user, end_node: session_1})

      {:ok, _} =
        Spelt.Repo.Relationship.create(%AuthenticatedAs{start_node: user, end_node: session_2})
